...
public class MokiMoviePlayerSample : MonoBehaviour
{
	...
	void Update()
	{
		...
		try
		{
			if (startedVideo && !mediaPlayer.Call<bool>("isPlaying")) {
				Application.LoadLevel("overlook-test_uv_Ali_15");
				//DoSwitchScene("overlook-test_uv_Ali_15");
				return;
			}
		}
		catch (Exception e)
		{
			Debug.Log("Failed to switch scene " + e.Message);
		}
		...
		{
			if (startedVideo && !movieTexture.isPlaying) {
				//Switch back
				Application.LoadLevel("overlook-test_uv_Ali_15");
				return;
			}
	...
	AndroidJavaObject StartVideoPlayerOnTextureId(int texWidth, int texHeight, string mediaPath)
	{
		...
		{
			Debug.Log("Failed to start mediaPlayer with message " + e);
		}
